...
- **ASCII-only in code, terminals, and any string that might hit `print()`.** This dev box is Windows; stdout defaults to cp1252 and `UnicodeEncodeError` crashes the script on the first non-ASCII glyph. Don't write true minus (`−`, U+2212), sigma (`σ`), em/en dash (`—`, `–`), smart quotes (`"`, `"`, `'`, `'`), bullets (`•`), arrows (`→`), or any other "pretty" Unicode in source files, log messages, format strings, or report renderers. Use ASCII: hyphen-minus `-`, `sigma`, `--`, straight quotes `"` `'`, `*`, `->`. If a renderer genuinely needs non-ASCII (e.g., a player name like `Yoán Moncada` pulled from data), the entry-point script must `sys.stdout.reconfigure(encoding="utf-8", errors="replace")` — but the default is plain ASCII.

- **Upstash is the source of truth for live data; the local SQLite KV is only as fresh as the last sync.** When analyzing, verifying, or reporting on live season state (standings, rosters, projections, Monte Carlo, SPoE, leverage), read from the remote Upstash KV, not `data/local.db` — the local cache can be days stale and silently produce wrong conclusions. Off Render, `get_kv()` returns the local SQLite store; to read Upstash directly, set `RENDER=true` (creds load from `.env`) before the first cache read, mirroring `scripts/refresh_remote.py`. `run_season_dashboard.py` also syncs remote->local at startup, but that clobbers not-yet-deployed local state (use `--no-sync` when verifying local changes).
...
- **`simulation.py` is the shared Monte Carlo module.** Use `run_monte_carlo()` / `run_ros_monte_carlo()` — don't rewrite MC loops.
