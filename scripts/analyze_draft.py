...
def main():
    ...
    print(
        f"  {'Mock' if meta['mock'] else 'Real'} draft | Position {meta['draft_position']} | "
        f"{meta['num_teams']} teams | {meta['strategy']} + {meta['scoring_mode']}"
    )
    print(f"  User team: {HART}")
    print(
        f"  Picks: {meta['picks_completed']}/{meta['total_picks']} | Complete: {meta['complete']}"
    )
    ...
    for r in mc_results:
        ...
        print(
            f"{r['team']:<32} {r['mean']:>5.1f} {r['std']:>5.1f} {r['median']:>4.0f} "
    ...
    print(
        f"  Mean: {hart_mc['mean']:.1f} pts | Median: {hart_mc['median']:.0f} | Std: {hart_mc['std']:.1f}"
    )
    print(
        f"  Win rate: {hart_mc['win_pct']:.1f}% | Top 3: {hart_mc['top3']:.1f}% | Bottom 3: {hart_mc['bot3']:.1f}%"
    )
    print(f"  Floor (P10): {hart_mc['p10']:.0f} | Ceiling (P90): {hart_mc['p90']:.0f}")
    print(
        f"  Average finish: {hart_mc['avg_finish']:.2f} (ranked {hart_mc_rank} of {len(mc_results)} by MC mean)"
    )
