...
def main() -> int:
    ...
    snapshot_date = ros_meta.get("_ros_snapshot_date")
    if ros_blob and snapshot_date:
        write_ros_projection_snapshot(get_kv(), ros_blob, snapshot_date)
        print(f"Archived ROS projection snapshot for {snapshot_date}.")
    else:
