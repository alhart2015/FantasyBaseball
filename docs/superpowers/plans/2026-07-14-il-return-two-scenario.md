...
class TestPlanIlReturnsScenarios:
    def _roster_with_il_hitter(self):
        """9 regular hitters + 3 pitchers (one BN) + an IL-slot returnee with a
        healthy preseason. counted = 13 (capacity), so activating the returnee
        forces exactly one drop (overflow 1)."""
        ...
        # IL-slot returnee: injury ROS (~175 PA), healthy preseason (~543 PA).
        ...
        return [*hitters, sp1, sp2, scrub, cruz]
    ...
    def test_suppressed_il_slot_returnee_produces_healthy_scenario(self):
        ...
        assert adj["vol_projected"] == pytest.approx(175.0, abs=1.0)
...
def test_il_return_plan_returns_scenario_envelope(client):
    ...
    with (
        ...
        patch("fantasy_baseball.web.season_routes.read_cache_dict",
              return_value={"projected_standings": {}, "team_sds": None,
...
def test_roster_audit_page_renders_il_returns_scenario_js(client):
    ...
    # The dual-scenario client logic is present.
