...
- SD-CEILING `test_ros_direct_uses_full_season_volume_for_cv_pt`: the `:695` `helper_sd < ...` ceiling rises slightly (variance now injected into the replacement fill there). VERIFY whether it actually trips -- a faithful replica measured `helper_sd` ~6.61 (det) -> ~6.67 (sampled, +1%) against a ceiling of ~11.85 (~1.8x headroom), so it is expected to STAY GREEN with large margin. Leave it UNCHANGED if green; re-pin ONLY if it actually trips, and then re-derive the bound preserving its intended headroom (do NOT tighten a passing guard toward the measured value). The `:690-691` healthy-iter exact-matches stay byte-identical -- do NOT touch them.
