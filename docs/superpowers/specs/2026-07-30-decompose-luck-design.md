...
- `composite()` and its weight tuples are driven by an explicit ordered
  `family_order` (defaulting to the shipped `FAMILIES`), validated against
  `KNOWN_FAMILIES`. Weight tuples become variable-length (`tuple[float, ...]`).
- `_qualified_families` computes ALL candidate `*_pct` columns
  (`value_pct`, `skill_pct`, `luck_pct`, `pt_pct`, `batted_ball_pct`, `age_pct`);
  `future_pct` is still attached by the caller. `family_order` selects the active
  subset. Compute-all-select-active keeps the two call sites (ranking, backtest)
  from validating different feature definitions.
...
- **Primary metric:** holdout Spearman rho vs next-year SGP percentile, for
  ...
  1.0). The highest holdout rho is the candidate winner.
...
- **Decision sniff test (reported + guard):** old->new rank moves for a watchlist
  -- Rafaela and Otto Lopez (should demote) and Alvarez (control, should not
  demote) -- plus mean realized next-year SGP of the winner's top-12 / top-20.

**Guard against a decision-nonsensical winner.** If the primary-metric winner
fails the sniff test (Rafaela/Otto NOT demoted relative to baseline, or Alvarez
demoted), the selection **stops and surfaces the full table to the user** rather
than shipping automatically. This preserves "let the holdout decide" while
guarding against the #276-style case where global rank correlation is maximized
but the top of the board is mis-ordered.
...
R9. Display schema (`SHOWN`) and `composite.py` docstrings are updated to the
    ...
    `FITTED_WEIGHTS` reference, string literal, and test.
