...
def test_transitions_for_matches_the_counts_the_spec_discloses() -> None:
    ...
    """
    from backtest_keeper_value import transitions_for
...
def test_normalizing_the_full_panel_survives_truncation_to_an_early_base_year() -> None:
    """The ordering rule, pinned on the pipeline `score_pool` actually runs.

    era_normalize RAISES without the 2023-2025 reference window, so normalizing a
    panel already truncated to 2022 aborts. score_pool slices the pre-normalized
    frame; this asserts that composition works for the earliest base year in scope.
    """
    from fantasy_baseball.trajectory.era import era_normalize

    full = era_normalize(_panel_2000_to_2026(), "hitter", sgp_overrides=None)
    truncated = full[full["season"] <= 2022]

    assert not truncated.empty
    assert int(truncated["season"].max()) <= 2022
    assert "era_factor_hr_pa" in truncated.columns
...
def test_without_player_removes_the_query_player() -> None:
    ...
    from backtest_keeper_value import without_player

    from fantasy_baseball.trajectory.era import era_normalize

    full = era_normalize(_panel_2000_to_2026(), "hitter", sgp_overrides=None)
    truncated = full[full["season"] <= 2024]
...
def test_horizons_for_drops_the_plus_two_run_where_2026_would_be_the_target() -> None:
    ...
    from backtest_keeper_value import horizons_for
...
def test_keeper_value_sgp_uses_the_panels_own_scorer() -> None:
    ...
    """
    from backtest_keeper_value import keeper_value_sgp
...
def test_var_uses_year_Y_eligibility_not_the_outcome_years(tmp_path: Path) -> None:
    ...
    from backtest_keeper_value import var_for
...
def test_censoring_boundaries(anchor, volumes, expected, why) -> None:
    from backtest_keeper_value import Outcome, censored
...
def test_a_missing_outcome_row_scores_zero_in_the_ALL_view() -> None:
    ...
    from backtest_keeper_value import Outcome
...
def test_outcomes_collapse_a_traded_players_split_season() -> None:
    ...
    from backtest_keeper_value import censored, outcomes_for
...
def test_resolve_draft_reports_an_unresolved_name_instead_of_dropping_it() -> None:
    ...
    from backtest_keeper_value import resolve_draft
...
def test_triple_regret_is_zero_when_the_forecast_picks_the_ex_post_best() -> None:
    from backtest_keeper_value import triple_regret
...
def test_agreement_rate_counts_identical_triples() -> None:
    ...
    from backtest_keeper_value import agreement_rate
...
def test_usable_draft_years_pins_the_headline_decision_counts() -> None:
    ...
    from backtest_keeper_value import usable_draft_years
...
def test_breakout_mask_selects_a_season_25_percent_over_the_prior() -> None:
    from backtest_keeper_value import breakout_mask
...
def test_the_roster_floor_is_applied_per_view_and_names_the_difference() -> None:
    ...
    from backtest_keeper_value import eligible_rosters
...
def test_bootstrap_difference_separates_an_obvious_gap() -> None:
    from backtest_keeper_value import bootstrap_difference
...
def test_a_starter_is_priced_against_the_SP_floor_not_the_RP_one(tmp_path: Path) -> None:
    ...
    """
    from backtest_keeper_value import slots_for
...
def test_pool_merge_keeps_a_two_way_player_under_one_pool_only() -> None:
    ...
    """
    from backtest_keeper_value import merge_pools
