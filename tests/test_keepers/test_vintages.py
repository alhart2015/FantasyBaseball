...
def test_zero_denominator_zips_rows_yield_nan() -> None:
    ...
    df = zips_pitchers(MLBAMID=33, IP=0.0, ER=0, BB=0, H=0, SO=0, W=0)
...
def test_zero_pa_hitter_rows_yield_nan() -> None:
    df = zips_hitters(MLBAMID=44, PA=0, AB=0, H=0, R=0, HR=0, RBI=0, SB=0)
...
def test_decompose_drops_rows_without_an_mlbam_id() -> None:
    df = zips_hitters(MLBAMID=[11, None])
...
def test_load_vintage_reads_real_files() -> None:
    ...
    hitters = load_vintage(2026, root, "hitter")
    pitchers = load_vintage(2026, root, "pitcher")
...
def test_both_loaders_decompose_to_the_same_rates() -> None:
    ...
    pd.testing.assert_frame_equal(
        decompose_hitters(zips_hitters(MLBAMID=5)),
        normalize_hitting(mlb_hitting(**{"player.id": 5})),
    )
    pd.testing.assert_frame_equal(
        decompose_pitchers(zips_pitchers(MLBAMID=6)),
        normalize_pitching(mlb_pitching(**{"player.id": 6})),
    )
...
def test_load_vintage_falls_back_to_a_year_suffixed_filename(tmp_path: Path) -> None:
    ...
    year = tmp_path / "2099"
    year.mkdir()
    zips_hitters(MLBAMID=1).to_csv(year / "zips-hitters-2099.csv", index=False)
    zips_pitchers(MLBAMID=2).to_csv(year / "zips-pitchers-2099.csv", index=False)
    assert list(load_vintage(2099, tmp_path, "hitter").index) == [1]
    assert list(load_vintage(2099, tmp_path, "pitcher").index) == [2]
...
def test_load_vintage_raises_when_no_export_exists(tmp_path: Path) -> None:
    ...
    with pytest.raises(FileNotFoundError, match="no ZiPS hitters export"):
        load_vintage(2098, tmp_path, "hitter")
