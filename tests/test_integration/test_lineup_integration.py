...
def preseason_standings():
    ...
    return _list_to_standings(
        [{"name": name, "rank": i + 1, "stats": dict(zero_stats)} for i, name in enumerate(teams)]
    )
...
def full_hitter_roster():
    ...
    return [
        ...
        _make_hitter("Sal Perez", ["C"], 58, 24, 78, 1, 0.253, 490),
        # 1B: Freddie Freeman type
        _make_hitter("Fred Freeman", ["1B"], 95, 28, 92, 10, 0.295, 565),
        # 2B: Marcus Semien type
        _make_hitter("Marc Semien", ["2B"], 88, 24, 75, 14, 0.268, 560),
        # 3B: Jose Ramirez type
        _make_hitter("Joe Ramirez", ["3B"], 98, 30, 105, 22, 0.278, 570),
        # SS: Trea Turner type
        _make_hitter("Trea Turner", ["SS"], 92, 21, 68, 28, 0.282, 555),
        # IF-eligible (2B/SS): Willy Adames type
        _make_hitter("Will Adames", ["SS", "2B"], 80, 26, 85, 8, 0.258, 530),
        # OF1: Aaron Judge type
        _make_hitter("Aaron Judge", ["OF"], 110, 48, 125, 4, 0.290, 540),
        # OF2: Juan Soto type
        _make_hitter("Juan Soto", ["OF"], 105, 35, 100, 6, 0.288, 520),
        # OF3: Kyle Tucker type
        _make_hitter("Kyle Tucker", ["OF"], 95, 30, 90, 18, 0.280, 545),
        # OF4: Luis Robert type
        _make_hitter("Luis Robert", ["OF"], 78, 25, 72, 15, 0.265, 480),
        # UTIL1: Yordan Alvarez type (DH)
        _make_hitter("Yordan Alvarez", ["DH"], 88, 35, 100, 1, 0.283, 510),
        # UTIL2: Shohei type (DH/OF)
        _make_hitter("Shohei Ohtani", ["DH", "OF"], 102, 42, 110, 15, 0.285, 545),
        # BN1: weaker OF
        _make_hitter("Bench OF", ["OF"], 55, 12, 45, 8, 0.248, 380),
        # BN2: backup C/1B
        _make_hitter("Bench UT", ["C", "1B"], 42, 10, 40, 2, 0.240, 320),
    ]
...
class TestLeverageIntegration:
    ...
    def test_tied_category_not_dominant(self, midseason_standings):
        ...
        tied_standings = Standings(
            effective_date=midseason_standings.effective_date,
            entries=modified,
        )
    ...
    def test_preseason_equal_weights(self, preseason_standings):
        ...
        leverage = calculate_leverage(preseason_standings, "Hart of the Order")
...
class TestHitterOptimizerIntegration:
    def test_all_hitter_slots_filled(self, midseason_standings, full_hitter_roster):
        ...
        expected_slot_count = sum(
            count for pos, count in DEFAULT_ROSTER_SLOTS.items() if pos not in ("P", "BN", "IL")
        )

        assert len(lineup) == expected_slot_count, (
            f"Expected {expected_slot_count} assigned slots, got {len(lineup)}: {lineup}"
        )
    ...
    def test_all_starters_have_non_negative_roto_delta(
        ...
        for a in lineup:
            assert a.roto_delta >= 0, f"{a.name} has negative roto_delta {a.roto_delta}"
    ...
    def test_multi_position_player_optimally_placed(self):
        ...
        hitters = [
            ...
            _make_hitter("Bobby Witt", ["SS"], 95, 28, 88, 30, 0.285, 560),
            # DH-only: can fill UTIL but not any positional slot
            _make_hitter("Yordan Alvarez", ["DH"], 85, 34, 98, 1, 0.281, 510),
            # Remaining roster: nobody else is SS-eligible
            _make_hitter("Catcher Guy", ["C"], 55, 18, 60, 2, 0.248, 440),
            _make_hitter("First Base Guy", ["1B"], 78, 26, 84, 3, 0.265, 520),
            _make_hitter("Second Base Guy", ["2B"], 72, 20, 70, 10, 0.262, 500),
            _make_hitter("Third Base Guy", ["3B"], 68, 22, 72, 5, 0.258, 490),
            _make_hitter("IF Guy", ["2B", "3B"], 65, 14, 55, 12, 0.255, 480),
            _make_hitter("OF Guy 1", ["OF"], 98, 38, 105, 6, 0.288, 550),
            _make_hitter("OF Guy 2", ["OF"], 88, 28, 82, 14, 0.275, 530),
            _make_hitter("OF Guy 3", ["OF"], 80, 22, 70, 18, 0.268, 510),
...
class TestPitcherOptimizerIntegration:
    def test_pitcher_lineup_respects_slot_count(self, midseason_standings, full_pitcher_roster):
        ...
        assert len(starters) == p_slots, f"Expected {p_slots} starters, got {len(starters)}"
        assert len(bench) == len(full_pitcher_roster) - p_slots, (
            f"Expected {len(full_pitcher_roster) - p_slots} bench pitchers, got {len(bench)}"
        )
    ...
    def test_pitcher_starters_have_non_negative_roto_delta(
        ...
        for s in starters:
            assert s.roto_delta >= 0, f"{s.name} has negative roto_delta {s.roto_delta}"
