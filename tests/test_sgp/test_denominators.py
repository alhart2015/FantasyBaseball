...
def test_overrides_specific_categories_string_keys():
...
def test_overrides_specific_categories_enum_keys():
    overrides = {Category.HR: 10.0, Category.SV: 8.0}
    denoms = get_sgp_denominators(overrides)
    assert denoms[Category.HR] == 10.0
    assert denoms[Category.SV] == 8.0
    assert denoms[Category.R] == DEFAULT_SGP_DENOMINATORS[Category.R]
...
def test_override_keys_normalized_to_enum():
    """String-keyed overrides must not leave bare-string keys in the result."""
    denoms = get_sgp_denominators({"HR": 10.0})
    assert all(isinstance(k, Category) for k in denoms)
    assert len(denoms) == len(DEFAULT_SGP_DENOMINATORS)
