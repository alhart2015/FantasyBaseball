...
def test_hitter_rest_of_season_deviation_sgp():
    ...
    # HR: (33 - 30) / 9 = 0.33
    assert result["HR"]["rest_of_season_deviation_sgp"] == pytest.approx(0.333, abs=0.01)
    # RBI: (85 - 90) / 20 = -0.25
    assert result["RBI"]["rest_of_season_deviation_sgp"] == pytest.approx(-0.25, abs=0.01)
    # SB: (12 - 10) / 8 = 0.25
    assert result["SB"]["rest_of_season_deviation_sgp"] == pytest.approx(0.25, abs=0.01)
    # AVG: (0.290 - 0.278) / 0.005 = 2.4
    assert result["AVG"]["rest_of_season_deviation_sgp"] == pytest.approx(2.4, abs=0.01)
...
def test_pitcher_rest_of_season_deviation_sgp():
    ...
    # K: (200 - 190) / 30 = 0.333
    assert result["K"]["rest_of_season_deviation_sgp"] == pytest.approx(0.333, abs=0.01)
    # ERA: (2.70 - 3.00) / 0.15 = -2.0, flip sign -> +2.0 (lower ERA = good)
    assert result["ERA"]["rest_of_season_deviation_sgp"] == pytest.approx(2.0, abs=0.01)
    # WHIP: (1.05 - 1.11) / 0.015 = -4.0, flip sign -> +4.0
    assert result["WHIP"]["rest_of_season_deviation_sgp"] == pytest.approx(4.0, abs=0.01)
