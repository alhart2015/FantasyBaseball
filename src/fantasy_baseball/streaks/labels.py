...
`HOT_ELIGIBLE_PT_BUCKET`. Cold is not gated.
...
# (issue #173). Only the high (20+ PA) bucket, where p90 is a genuine top-decile
# count, is eligible for a "hot" label. Enforced by exact equality (this is the
# single eligible bucket, not an ordered floor), so it is the top bucket in
# ``windows.PT_BUCKETS``. Cold is not gated: sparse cold is skill-relative Poisson
# (self-guards at low PA via a ~0 expectation) and dense cold is left as is.
#
# A window that clears p90 but is sub-high is NEUTRAL, never cold: a high count is
# not a cold streak, so the hot gate must not spill would-be-hot windows into the
# cold branch (it would in a degenerate p10 >= p90 bucket).
HOT_ELIGIBLE_PT_BUCKET: PtBucket = "high"
        ...
        sql = f"""
            ...
            SELECT
                ...
                CASE
                    ...
                    WHEN w.{category} >= t.p90 AND w.pt_bucket = '{HOT_ELIGIBLE_PT_BUCKET}'
                        THEN 'hot'
                    WHEN w.{category} >= t.p90 THEN 'neutral'
...
def _apply_sparse_labels(conn: duckdb.DuckDBPyConnection, *, season_set: str) -> int:
    ...
    for category in SPARSE_CATEGORIES:
        ...
        # A window clears p90 if its count is top-decile for the stratum. Hot only
        ...
        for cold_method, percentile in POISSON_PERCENTILES:
            ...
            # A window that clears p90 is never cold, even when the hot gate
            # neutralizes it for being sub-high (a high count is not a cold
