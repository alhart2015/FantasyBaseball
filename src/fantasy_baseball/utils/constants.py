...
# Closer role-switch mixture curves (SV variance). Smooth functions of projected SV:
#   q(s)   = sigmoid(b0 + b1*s)          -- probability of the modal (role-unchanged) outcome
#   a_s(s) = max(0, c0 + c1*s)           -- surprise multiplier (job loss ~0 at high s,
#                                           vault-in large at low s); a_m derived from mean-1.
