...
# Closer role-switch mixture curves (SV variance). Two logistic functions of projected SV:
#   q(s) = sigmoid(b0 + b1*s)   -- probability of the modal (role-unchanged) outcome
#   f(s) = sigmoid(g0 + g1*s)   -- the surprise component's share of the unit mean
# giving a_s = f/(1-q), a_m = (1-f)/q (mean-1 and non-negative by construction).
# PROVISIONAL scaffold; regenerated by scripts/calibrate_closer_mixture.py.
# See src/fantasy_baseball/sgp/closer_mixture.py and the design spec.
SV_ROLE_MIXTURE: dict[str, list[float]] = {
    "q_logit": [2.3130, -0.1731],
    "f_logit": [-0.1355, 0.1504],
}
